...
public class Study {
    ...
    public static void main(String args[]) {
        ...
        // その理由をコメントへ記述してください。
        // getメソッドで増減させると、"月の値＋数値"となり、12月以降が表示されるため。
        ...
        int ctmonth = calendar.get(Calendar.MONTH);
        int ctdate = calendar.get(Calendar.DATE);
        
        int addy = 1;
        int addm = 2;
        int addd = 15;
        
        calendar.add(Calendar.YEAR, addy);
        calendar.add(Calendar.MONTH, addm);
        calendar.add(Calendar.DATE, addd);
        
        //年越し、月またぎを想定
        int lastmonth = 12;
        int lastday = calendar.getMaximum(Calendar.DAY_OF_MONTH);  
        
        if(ctmonth + addm > lastmonth) {
        	calendar.add(Calendar.YEAR, (ctmonth + addm) / lastmonth);	
        }
        if(ctdate + addd > lastday) {
        	 calendar.add(Calendar.MONTH, (ctdate + addd) / lastday);
        }
        //
